...
function App() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:313/register?username=${user?.username}`)
      .then((response) => {
        setUser(response.data[0]?.id);
        axios
          .get(`http://localhost:313/wishlist?Uid=${response.data[0]?.id}`)
          .then((res) => {
            setrow(res);
            setLoader(false);
          });
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:313/address?email=${user?.id}`)
      .then((response) => {
        const { data } = response;
        setAddress(data);
      });
